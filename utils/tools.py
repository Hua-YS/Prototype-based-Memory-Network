...
def SetData_single(dataset, im_size, tr_test=0.8):
    ...
    return X_tr, y_tr, X_test, y_test, X_ori
...
def SetData_multi(dataset, im_size, one_shot=False):
    ...
    for i in range(len(filenames_trainval)):
        ...
        im = np.float32(Image.open(im_path+trainval_folder+filenames_trainval[i]).resize((im_size, im_size)))
        im = im[:, :, [2, 1, 0]]
        im_copy = im.copy()
        im[:,:,0] -= 103.939
        im[:,:,1] -= 116.779
    ...
    for i in range(len(filenames_test)):
        ...
        im = np.float32(Image.open(im_path+test_folder+filenames_test[i]).resize((im_size, im_size)))
        im = im[:, :, [2, 1, 0]]
        im_copy = im.copy()
        im[:,:,0] -= 103.939
        im[:,:,1] -= 116.779
